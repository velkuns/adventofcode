...
class Day4 implements AlgorithmInterface
{
    ...
    {
        $passports = array_filter($this->parsePassports($inputs), [$this, 'isValidPassport']);
        return (string) count($passports);
    }

    private function starTwo(array $inputs): string
    {
        $passports = array_filter($this->parsePassports($inputs), [$this, 'isValidFullPassport']);
        return (string) count($passports);
    }

    private function parsePassports(array $inputs): array
    {
        $passportIndex = 0;
        $passports     = [];
    ...
    {
        static $mandatory = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid'];// 'cid'
    ...
    private function isValidFullPassport(array $passport): bool
    {
        ...
        $value = (int) $passport['hgt'];
        $type  = substr($passport['hgt'], -2);

        if ($type !== 'in' && $type !== 'cm') {
            return false;
        }

        $isValidCm = ($type === 'cm' && ($value >= 150 && $value <= 193));
        $isValidIn = ($type === 'in' && ($value >= 59 && $value <= 76));
        if (!$isValidCm && !$isValidIn) {
            return false;
        }

        if (!(bool) preg_match('`^#[[:xdigit:]]{6}$`', $passport['hcl'])) {
            return false;
        }
        ...
        if (!(bool) preg_match('`^[[:digit:]]{9}$`', $passport['pid'])) {
            return false;
        }
