...
class Day6 implements AlgorithmInterface
{
    public function getExamples(string $star): array
    {
        $examples = [
            '*'  => [
                [
                    11 => ['abc', '', 'a', 'b', 'c', '', 'ab', 'ac', '', 'a', 'a', 'a', 'a', '', 'b'],
                ]
            ],
            '**' => [
                [
                    6 => ['abc', '', 'a', 'b', 'c', '', 'ab', 'ac', '', 'a', 'a', 'a', 'a', '', 'b'],
                ]
            ],
        ];
    ...
    private function starOne(array $inputs): int
    {
        ...
        foreach ($inputs as $input) {
            if (empty($input)) {
                $groups[] = count(count_chars($answers, 1));
                $answers = '';
                continue;
            }
    ...
    private function starTwo(array $inputs): int
    {
        ...
        foreach ($inputs as $input) {
            if (empty($input)) {
                $groups[] = count(array_filter(count_chars($answers, 1), function ($nb) use ($nbInGroup) { return $nb === $nbInGroup;}));
                $nbInGroup = 0;
                $answers   = '';
                continue;
            }
