...
class Day1 implements AlgorithmInterface
{
    ...
    {
        $init = ['p' => array_shift($inputs), 'c' => 0];
        $out  = array_reduce($inputs, fn (array $out, $val) => ['p' => $val, 'c' => $out['c'] + ($val > $out['p'])], $init);
    ...
    {
        $groups = [];
        for($i = 0, $max = count($inputs) - 2; $i < $max; $i++) {
            $groups[] = array_sum(array_slice($inputs, $i, 3));
        }

        $init = ['p' => array_shift($groups), 'c' => 0];
        $out  = array_reduce($groups, fn (array $out, $val) => ['p' => $val, 'c' => $out['c'] + ($val > $out['p'])], $init);
